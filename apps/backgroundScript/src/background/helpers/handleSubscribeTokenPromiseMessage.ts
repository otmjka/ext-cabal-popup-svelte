import { SubscribeTokenPromisePayloadMessage } from '@/shared/src/cabalSharedTypes';
...
export const handleSubscribeTokenPromiseMessage = async ({
	...
	try {
		...
		if (!listener) {
			throw new Error('[handleSubscribeTokenPromiseMessage] cant create listener');
		}

		listener.mint = messageMint;

		if (!state.readyPromise) {
			throw new Error('use before installation state.readyPromise is undefined');
		}
