import type { StrBigInt, TokenStatusParsed, TradeStatsParsed } from '../cabalSharedTypes';
import { parsedNumberSchema } from './parsers';
import { oneTokenPriceInSol } from './token';

export enum TargetFormatType {
	mc = 'mc',
	sol = 'sol',
	usd = 'usd'
}
...
export const getLimitPriceFormatted = ({
	formatType,
	solPrice,
	tokenStatusSupply,
	solPriceUSD
	...
	try {
		const supply = parsedNumberSchema.parse(tokenStatusSupply) / 1e9;
		if (formatType === TargetFormatType.mc) {
			if (solPrice && supply && solPriceUSD) {
				const mcUsd = solPrice * supply * solPriceUSD;
				const mc = formatMC(mcUsd);
				return mc;
			}
		}
		if (formatType === TargetFormatType.sol) {
			if (solPrice) {
				const priceSol = Number(solPrice).toFixed(3);

				return priceSol;
			}
		}
		if (formatType === TargetFormatType.usd) {
			if (solPrice && solPriceUSD) {
				const priceSol = Number(solPrice * solPriceUSD).toFixed(3);
